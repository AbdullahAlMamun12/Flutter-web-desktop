...
class _FilesAppState extends ApplicationState {
    ...
    return Container(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          leading: Row(
            children: [
              IconButton(onPressed: (stack.length>1)? _onBackClicked : null, icon: const Icon(Icons.arrow_back_ios))
            ],
          ),
  ...
  void initState() {
    super.initState();
    _fileManager = getx.Get.find<FileManager>();
  ...
  final TreeViewTheme _treeViewTheme = const TreeViewTheme(
  ...
  Widget _nodeBuilder(BuildContext buildContext, Node<dynamic> node) {
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              Text(node.data.name,style: const TextStyle(color: Colors.white),),
            ],
          ),
        );
  }
